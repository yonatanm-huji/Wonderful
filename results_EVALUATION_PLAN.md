...
## due to time constraints, I will not be able to complete a full test of the agent. However, some initial testing showed that the agent struggled with calling multiple tools, particularly in a single step. Using ReAct or another multi-step agent frameworks could resolve these issues.

All tools function correctly when called. Limitations arise from LLM function-calling behavior, not the tools themselves.

Limitations:

1. Inconsistent Multi-Tool Orchestration
Issue: Agent doesn't reliably call multiple related tools in the same response, even when both are necessary.

Example: When asked "I'm Jalen Brunson, can I pick up Amoxicillin?", the agent may call get_medication_info but not get_user_allergies, missing a critical Penicillin allergy check.

Why: LLMs choose tools probabilistically. The medication name creates a stronger signal than the implied need for safety checks. The model may call one tool, verbally state it will check allergies, but never actually call the allergy tool.


2. Tool Result Mistrust
Issue: Agent sometimes calls a tool successfully but then tells the user it "cannot check that information directly" and refers them to pharmacy staff, ignoring the tool result it just received.

Example: User asks "Do I have a valid prescription?" → Agent calls check_prescription → Tool returns result → Agent says "I don't have the capability to check prescription records directly."

Why: LLM is overly cautious about medical/pharmacy tasks and doesn't trust its own tool results, erring on the side of safety by redirecting to human pharmacists.

Impact: Tools work correctly, but user experience is degraded by unnecessary referrals.

3. Query Phrasing Sensitivity
Issue: Questions phrased as hypothetical situations trigger medical advice filter instead of factual information lookup.
Examples:

"If I'm taking Warfarin, can I take Aspirin?" → Triggers refer_to_professional
"What does Warfarin interact with?" → Calls check_active_ingredients_and_interactions

Why: The model interprets "if I'm taking" as a request for personalized medical advice rather than general drug information.
...
RESULTS - test failed.
- the agent failed to both look up the patient and the drug. This is likely due to the limitations of The agent needed to: 
1) look up the patient to understand: 
	a) if they had a prescription 
	b) if they had any allergies 
2) look up amoxicillin to see:
	a) its ingredients to check for potential allergies & interactions
	b) if it had any inventory, using a separate Inventory tool

As a result, the agent was first willing to prescribe amoxicillin to Jalen Brunson, despite his allergy. In a second attempt, the agent began to hallucinate and made multiple calls to the same tool in a single step. 
...
RESULTS - multi-step test successful. Single-step partially-successful.

-the agent first calls the inventory tool, sees that aspirin is in stock
-the agent then checks the interaction tool, sees that warfarin and aspirin do not interact well
-finally the agent calls the refer to health professional tool to find an alternative

In the single-step; which is written above:
-the agent uses the get medication info tool, which contains enough context to mention that warfarin and aspirin do not interact well.
...
-Retrieved information about semaglutide/wegovy
-recalled the context in the following steps
-failed to call the inventory tool, instead calling the medication info tool to call for allergies
...
## 12. EXAMPLE Evaluation Summary Template
...
**Conclusion:**
This will determine if Duane "the Rock" Reade pharmacy AI agent successfully demonstrates:
...
If so, the agent is suitable for demonstration and meets all assignment requirements.
